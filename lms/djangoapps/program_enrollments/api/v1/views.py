...
def verify_program_exists(view_func):
    """
    Raises:
        An API error if a view function's `program_key` argument is null,
        or if no such key exists in the catalog programs cache.
    """
    @wraps(view_func)
...
class ProgramEnrollmentsView(DeveloperErrorViewMixin, PaginatedAPIView):
    ...
    The path can contain an optional `page_size?=N` query parameter.  The page size is 100.
